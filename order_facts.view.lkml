...
view: order_facts {
  ...
  dimension: is_before_mtd {
    type: yesno
    sql: (EXTRACT(DAY FROM ${created_date}) < EXTRACT(DAY FROM ${max_date}))
      ;;
  }
  ...
  measure: projected_order_count {
    ...
    html: {% if created_month._value ==  max_month._value %}
  ...
  set: detail {
    fields: [id, users.last_name, users.first_name, users.id, order_items.count]
  }
}
